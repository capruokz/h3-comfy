...
if [ "${1:-}" = "--fix" ]; then
  # เลือก torch ตามการ์ดที่เจอจริง ไม่ใช่ยัดเวอร์ชันเดียวให้ทุกใบ
  # การ์ด Blackwell (sm_120) ต้อง cu130 -- วัดแล้วว่า cu128 ช้ากว่า 1.84 เท่า
  # การ์ดรุ่นก่อนหน้าไม่ผูกกับเวอร์ชันไหน ถ้าของเดิมใช้ได้อยู่แล้วก็ไม่ต้องยุ่ง
  # ชุดนี้รองรับเฉพาะ Blackwell จึงมีเป้าหมายเดียวคือ cu130
...
ขั้นต่อไป — หาเพดานของการ์ดใบนี้ก่อนใช้งานจริง:
  python3 find_ceiling.py

ตัวเลขความยาวคลิปสูงสุดในคู่มือวัดมาจากการ์ด 16 GB ใบหนึ่ง ไม่ใช่ความจริงของทุกใบ
find_ceiling.py จะหาเพดานของเครื่องคุณเองแล้วเขียนลง machine.json
EOF
