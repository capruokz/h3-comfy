...
def _unique_t(timestep, shift_v, shift_a, has_vis_cond, has_aud_cond=False):
    # must produce exactly the row set of comfy/ldm/minimax/model.py's `unique_t`,
    # or the injected [M, out] update cannot be added to the [M', out] projection.
    # ref2va with a standalone <Audio j> adds a fourth kind of row pinned at
    # AUDIO_COND_TIMESTEP (1.0), distinct from the visual one at 0.999.
...
def _inject_adaln_egrid(new_model, dm, lora, adaln, strength):
    ...
    def wrap(executor, *args, **kwargs):
        ...
        # read the cond latent lists, not the raw keyframes/refs: an audio-only
        # reference makes payload["refs"] truthy while contributing no visual row
        has_vc = bool(payload.get("cond_video_latents"))
        has_ac = bool(payload.get("cond_audio_latents"))
        us = _unique_t(ts, shift_v, shift_a, has_vc, has_ac)
